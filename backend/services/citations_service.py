"""
Citations service for KNO.

Responsibilities:
- Extract citation metadata from document chunks (heuristic regex-based;
  no LLM call needed for raw reference strings).
- Manually add / edit / delete citations stored in DB.
- Format citations in IEEE, APA, MLA, Chicago, and BibTeX styles.
"""
...
# ---------- Heuristic reference-string extraction ----------

# Common patterns:
# [1] Author, A. (2020). Title. Venue.
# 1. Author, A., "Title", Venue, 2020.
# Author, A. et al. (2020). Title. arXiv:2001.12345.
YEAR_RE = re.compile(r"(19|20)\d{2}")
ARXIV_RE = re.compile(r"arXiv\s*[:\.]\s*(\d{4}\.\d{4,5}(v\d+)?)", re.IGNORECASE)
DOI_RE = re.compile(r"(?:doi[:\s]*|https?://(?:dx\.)?doi\.org/)(10\.\d{3,9}/[^\s,;)]+)", re.IGNORECASE)
...
def extract_citation_candidates(document_id: str) -> List[Dict]:
    """
    Look through document text for reference lines and return rough
    citation candidates. This is intentionally heuristic and lightweight —
    no LLM call. The user can confirm/edit/add manually.
    """
    ...
    RE_HEAD = re.compile(
        r"(?m)^\s*(?:\[\d+\]|\d+\.|\(\d+\))?\s*([A-Z][A-Za-zÀ-ž\-\.\']+"
        r"(?:,\s*[A-Z]\.\s*[A-Za-zÀ-ž\-\.\']+){0,4})"
    )

    for chunk in chunks:
        text = chunk.get("text", "") or ""
        # Look at last few chunks (references are typically near the end)
        for line in text.splitlines():
            ...
            # Heuristic title (between year and venue-ish)
            title = None
            if year:
                parts = line.split(str(year), 1)
                if len(parts) == 2:
                    tail = parts[1].lstrip(",. ").rstrip(".")
                    # First 8-12 words after year often the title
                    words = tail.split()
                    title_words = []
                    for w in words[:14]:
            ...
            if not title:
                # Just use the lead of the line as the title candidate
            ...
            candidates.append({
                ...
                "venue": None,
                "doi": doi,
                "url": f"https://doi.org/{doi}" if doi else (f"https://arxiv.org/abs/{arxiv_id}" if arxiv_id else None),
                "arxiv_id": arxiv_id,
                "raw": _truncate_at(line, 400),
...
def format_bibtex(c: Dict) -> str:
    ...
    first_word = re.sub(r"[^A-Za-z0-9]", "", first_author_last.lower()) or "ref"
    key = f"{first_word}{year}" if year else first_word
    # Add a short entropy from external_id / id to avoid duplicate keys
...
def format_citation(c: Dict, style: str) -> str:
    ...
    if not fmt:
        # Default to IEEE if unknown style
        fmt = format_ieee
    return fmt(c)
