...
Uses the HuggingFace Datasets public API (free, no key). Falls back
gracefully if HuggingFace is unreachable. Datasets from Kaggle require
credentials and are not implemented here.
"""
...
def _hf_search(query: str, limit: int = 10) -> List[Dict]:
    ...
    for d in data or []:
        results.append({
            "id": d.get("id"),
            "description": None,  # HF list endpoint doesn't include descriptions
