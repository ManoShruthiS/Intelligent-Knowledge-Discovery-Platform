...
Uses the public GitHub Search API. No key required for low-volume use
(60 req/hr per IP). A `GITHUB_TOKEN` env var dramatically raises the
rate limit and is recommended for deployment.
"""
...
def _github_search_repos(query: str, limit: int = 10) -> Dict:
    ...
    for r_item in items:
        results.append({
            "id": r_item.get("id"),
            "full_name": r_item.get("full_name"),
            "description": r_item.get("description"),
            "html_url": r_item.get("html_url"),
            "language": r_item.get("language"),
            "stars": r_item.get("stargazers_count"),
            "forks": r_item.get("forks_count"),
            "open_issues": r_item.get("open_issues_count"),
            "updated_at": r_item.get("updated_at"),
            "topics": r_item.get("topics") or [],
            "license": (r_item.get("license") or {}).get("spdx_id") if isinstance(r_item.get("license"), dict) else None,
            "source": "github",
        })
    return {"results": results, "available": True, "rate_limit": "ok", "total": data.get("total_count", len(results))}


def search_repositories(query: str, limit: int = 10) -> Dict:
    return _github_search_repos(query, limit)
